...
if (!(is.null(test.df))) {
  preds.on.test = predict(model, newdata = test)
  perf.on.test = performance(preds.on.task, measures.perf, model = model,
    task = task)  
}
...
if (!(is.null(test.df))) {
  ...
  mod.test.perf = kable(perf.on.test)
  out.mod = c(out.mod, mod.text.test, mod.test.perf)
}
```
